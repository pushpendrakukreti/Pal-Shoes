...
public partial class viewallproducts : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (!IsPostBack)
        {
            ...
            string viewall = Request.QueryString["viewall"];
            if (viewall != null)
            {
                BindViewAllProducts(viewall);
            }                        
    ...
    private void BindViewAllProducts(string productalltype)
    {
        ...
        try
        {
            ...
            dt = local_product.getViewAllProducts(productalltype, page, out howManyPages);
            if (dt.Rows.Count > 0)
            {
                rpProduct.DataSource = dt;
                rpProduct.DataBind();

                firstPageUrl = Link.ToAllProductOnDefault("1");
                pagerFormat = Link.ToAllProductOnDefault("{0}");
            }
