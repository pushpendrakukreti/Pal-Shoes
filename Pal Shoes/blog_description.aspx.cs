...
public partial class blog_description : System.Web.UI.Page
{
    ...
    private void BindAdminBlogDetails(string blogid)
    {
        DataTable dt = new DataTable();
        try
        {
            ...
            if (dt.Rows.Count > 0)
            {
                rpBlogList.DataSource = dt;
                rpBlogList.DataBind();
            }
            else
            {
                lblBlogList.Text = "<br/><br/><br/>There is no any records.";
                lblBlogList.Font.Size = 20;
            }
        }
