...
public partial class product : System.Web.UI.Page
{
            ...
            if (categoryid == null && subcategoryid == null && flavour == null && brand == null)
            {
                if (ddSortBy.SelectedIndex > 0)
                {
                    if (ddSortBy.SelectedValue == "1")
                    {
                        ...
                        BindProductsLowToHighByCategoryId(categoryid);
                    }
                    else if (ddSortBy.SelectedValue == "2")
                    {
                        BindProductsHighToLow();
                        BindProductsHighToLowByCategoryId(categoryid);
                    }
                }
                else
                {
    ...
    private void BindDefaultProducts()
    {
        ...
        try
        {
            ...
            if (dt.Rows.Count > 0)
            {
                ...
                lblProductType.Text = "Offer Products";
    ...
    private void BindProductBySubcategory(string subcategoryid)
    {
        ...
        try
        {
            ...
            if (dt.Rows.Count > 0)
            {
                ...
                BindNavProductName2(subcategoryid);
            }
            bottomPager.Show(int.Parse(page), howManyPages, firstPageUrl, pagerFormat, true);
        }
        catch
    ...
    private void BindProductByBrand(string brand)
    {
        ...
        try
        {
            ...
            if (dt.Rows.Count > 0)
            {
                ...
                lblProductType.Text = brand + " Brand";
                lblProductSubCategory.Text = "";
                lblProductCategory.Text = "";
            }
            bottomPager.Show(int.Parse(page), howManyPages, firstPageUrl, pagerFormat, true);
        }
    ...
    protected void linkAddtoCart_Click(object sender, EventArgs e)
    {
        ...
        try
        {
            ...
            if (dt.Rows.Count > 0)
            {
                ...
                int result = 0;
                result = local_cart.InsertIntoShoppingCart(transid, productid, price, variantname, variantquantity, quantity, discount, totalprice, shippingcharge, name, thumbnail, capacity, userid, flavour1);
                if (result != 0)
                {
                    string subcategoryid = Request.QueryString["subcategory"];
                    string categoryid = Request.QueryString["category"];
                    ...
                    if (subcategoryid != null)
                    {
                        Response.Redirect("~/product.aspx?subcategory=" + subcategoryid + "", false);
                    }
                    else if (categoryid != null)
                    {
                        Response.Redirect("~/product.aspx?category=" + categoryid + "", false);
                    }
                    else if (flavour != null)
                    {
                        Response.Redirect("~/product.aspx?Flavour=" + flavour + "", false);
                    }
                    else if (brand != null)
                    {
                        Response.Redirect("~/product.aspx?Brand=" + brand + "", false);
                    }
                    else
                    {
    ...
    private void BindProductsLowToHighByCategoryId(string categoryid)
    {
        string page = Request.QueryString["Page"];
        if (page == null)
            page = "1";
        ...
        {
            DataTable dt = new DataTable();
            dt = local_product.getLowToHighByCategoryId(categoryid, page, out howManyPages);
            if (dt.Rows.Count > 0)
            {
                rpProduct.DataSource = dt;
                rpProduct.DataBind();
    ...
    private void BindProductsHighToLowByCategoryId(string categoryid)
    {
        string page = Request.QueryString["Page"];
        if (page == null)
            page = "1";
        ...
        {
            DataTable dt = new DataTable();
            dt = local_product.getHighToLowByCategoryId(categoryid, page, out howManyPages);
            if (dt.Rows.Count > 0)
            {
                rpProduct.DataSource = dt;
                rpProduct.DataBind();
    ...
    private void BindNavProductName(string categoryid)
    {
        DataTable dt = new DataTable();
        try
        {
            dt = local_product.getProductNavName(categoryid);
            if (dt.Rows.Count > 0)
            {
                lblProductCategory.Text = Convert.ToString(dt.Rows[0]["CategoryName"]);
                lblProductSubCategory.Text = "";
                lblProductType.Text = "";
            }
        }
    ...
    private void BindNavProductName2(string subcategoryid)
    {
        DataTable dt = new DataTable();
        try
        {
            dt = local_product.getProductNavName2(subcategoryid);
            if (dt.Rows.Count > 0)
            {
                lblProductCategory.Text = Convert.ToString(dt.Rows[0]["category"]);
                lblProductSubCategory.Text = "/ " + Convert.ToString(dt.Rows[0]["SubName"]);
                lblProductType.Text = "";
            }
        }
        catch
