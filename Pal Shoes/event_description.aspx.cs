...
public partial class event_description : System.Web.UI.Page
{
    ...
    private void BindAdminEventDetails(string eventid)
    {
        DataTable dt = new DataTable();
        try
        {
            ...
            if (dt.Rows.Count > 0)
            {
                rpEventList.DataSource = dt;
                rpEventList.DataBind();
            }
            else
            {
                lblEventList.Text = "<br/><br/><br/>There is no any records.";
                lblEventList.Font.Size = 20;
            }
        }
